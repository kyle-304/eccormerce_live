defmodule EcommerceLiveWeb.UserLive.Settings do
  use EcommerceLiveWeb, :live_view

  on_mount {EcommerceLiveWeb.UserAuth, :require_sudo_mode}
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      <div class="text-center">
        <.header>
          Account Settings
          <:subtitle>Manage your account email, password, and phone settings</:subtitle>
        </.header>
      </div>
      ...
      <.form for={@phone_form} id="phone_form" phx-submit="update_phone" phx-change="validate_phone">
        <.input field={@phone_form[:phone]} type="tel" label="Phone" autocomplete="tel" required />
        <.button variant="primary" phx-disable-with="Updating...">Update Phone</.button>
      </.form>
  ...
  def handle_event("update_email", %{"user" => user_params}, socket) do
    ...
    case Accounts.change_user_email(user, user_params) do
      %{valid?: true} = changeset ->
        ...
        {:noreply, put_flash(socket, :info, "A confirmation link was sent to your new email.")}
  ...
  def handle_event("update_phone", %{"user" => user_params}, socket) do
    ...
    case Accounts.update_user_phone(user, user_params) do
      {:ok, _user} ->
        {:noreply, put_flash(socket, :info, "Phone updated successfully.")}
